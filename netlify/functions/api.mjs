...
**המשימה שלך:** זהה 3–4 צירי מתח מרכזיים שעולים מהשוואת תשובות הקבוצות. לכל ציר — כתוב 3–4 שאלות עבודה הנוגעות ישירות בתפקיד המנהל.

**מבנה הפלט — MUSTBE JSON:**
הוצא את התוצאה כ-JSON valid בהמעט \`\`\`json ו-\`\`\`שבסופו.

\`\`\`json
{
  "tensions": [
    {
      "title": "שם הציר — מנוסח כמתח בין שני קטבים (צפוי: 3-4 מילים)",
      "description": "משפט קצר המתאר את המתח כפי שעלה מהנתונים",
      "questions": [
        "שאלה 1",
        "שאלה 2",
        "שאלה 3",
...
**כללים:**
- השאלות נוגעות לתפקיד המנהל בפועל — לא שאלות תיאורטיות
- כל שאלה פתוחה, ללא תשובה נכונה אחת
- השאלות נובעות ישירות מהנתונים — השתמש בדוגמאות ספציפיות מהתשובות
- אל תיתן תשובות, פתרונות או המלצות — רק שאלות
- עברית ישירה, קצרה, חדה`;
}
